...
![alt text](1-success.jpg)
![alt text](1-not-found.jpg)
...
#### Решение
Первый бит не уйдет до тех пор, пока не соберется хотя бы один пакет. 
Пакет будет собираться 56 * 8 / 128 / 1000 секунд = 0.0035 секунды.
Затем этот пакет будет передаваться по линии связи между хостами в течение: 56 * 8 / 1'000'000 = 0.000448 секунды. После этого еще добавится задержка распространения 5 мс = 0.005 секуды.
В сумме получим время: 0.0035 + 0.000448 + 0.005 = 0.00894 секунды.
Ответ: 0.00894 секунды
...
#### Решение
N = 1 + 10 = 11
Средняя задержка ожидания = 10 мс
Средняя задержка передачи = 1 / 100 секунд = 10 мс
d = Средняя задержка ожидания + Средняя задержка передачи = 20 мс
a = N / d = 11 / 20 мс = 0.55 пакетов в милисекунду = 550 пакетов в секунду
Ответ: 550 пакетов в секунду
