...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - 192.168.88.35
   - 38608
   - ![alt text](<Thu Apr  4 11:33:21 AM MSK 2024.jpg>)
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 0
   - флаг SYN
   - ![alt text](<Thu Apr  4 11:37:11 AM MSK 2024.jpg>)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - В заголовке есть флаги SYN и ACK
   - ![alt text](<Thu Apr  4 11:39:08 AM MSK 2024.jpg>)
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - 1
   - ![alt text](<Thu Apr  4 11:41:32 AM MSK 2024.jpg>)
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Номера первых 6 сегментов: 1, 2063, 4983, 7903, 10823, 15203
   - 5.4960718, 5.5427552, 5.5931705, 5.6217743, 5.6510365, 25.8792853 все в секундах от начала перехвата
   - 5.5409228, 5.5915769, 5.6202490, 5.6495401, 5.6925098, 25.8851595 все в секундах от начала перехвата
   - RTT = 0.032644083333366325 секунды = 32 мс
   ![alt text](<Thu Apr  4 11:45:48 AM MSK 2024.jpg>)
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   времени) для этого TCP-соединения? Объясните, как вы получили это значение.
      - Посмотрим на размер окна. Оно имеет размер 64256 байт.
   - Тогда по формуле получаем $\frac{64256}{0.032} = 2008000 $ байт в секунду $ = 2 $ мбайт / секунду.
   - ![alt text](<Thu Apr  4 12:06:10 PM MSK 2024.jpg>)
...
#### Скрин
![alt text](<Thu Apr  4 12:11:41 PM MSK 2024.jpg>)
...
По условию $L = 1500 \text{ байт} = 1500 * 8 \text{ бит} = 12000 \text{ бит}$.
$R = 10^9$ бит/с. $RTT = 30$ мс $ = 0.03$ с.
По формуле $U=n \cdot \dfrac{L/R}{RTT + L/R}$. Тогда надо решить неравенство $U > 0.98 \leftrightarrow n > 0.98 \cdot (RTT + L / R) / (L / R)  \leftrightarrow n > 0.98 \cdot 2501 \leftrightarrow n > 2450.98$.

Ответ: $n \geqslant 2451$.
