...
#### Решение
Средняя пропускная способность равна TCP $K = 1.22 \frac{MSS}{RTT \cdot \sqrt{L}}$. Отсюда получаем $L = 1.22^2 \cdot \frac{MSS ^ 2}{K ^ 2 \cdot RTT ^ 2}$ - коэффициент потерь. Временной интервал $T = \frac{1}{L} \cdot \frac{MSS}{K}$ = $\frac{K ^ 2 \cdot RTT ^ 2}{1.22 ^ 2 \cdot MSS ^ 2} \cdot \frac{MSS}{K} = \frac{K \cdot RTT ^ 2}{1.22 ^ 2 \cdot MSS}$. Как видим, К не сократилась, а это значит, что T это функция от средней пропускной способности TCP.
...
#### Решение
1. Установление соединения занимает $RTT$ + отправка первого запроса $RTT$ + получение первого сегмента данных $\frac{S}{R}$. + RTT между отправкой ACK и получением нового сегмента данных. Затем окно увеличивается в 2 раза и тратим время $\frac{S}{R} + RTT$ для нового ACK и фрагмента данных. Третий ACK приходит во время отправки третьего окна, снова увеличившего в 2 раза. Суммарно потратим время:
$4RTT + 14 \frac{S}{R}$


2. Аналогично пункту 1, только ACK приходит уже после завершения третьего окна, поэтому время $5RTT + \frac{15S}{R}$ 

3. Аналогично пункту 1, т.к. $\dfrac{𝑆}{𝑅} > 𝑅𝑇𝑇$, то уже начиная с окна размера 2 пакет приходит во время отправки текущего окна, поэтому окна пойдут подряд, из-за чего общее время $3RTT + \frac{15S}{R}$
...
#### Решение
После первого подтверждения размер окна перегрузки будет $W /2 * (1 + a)$.

После втого  $W / 2 * (1 + a) ^ 2$.

В конце получим $W$.

Следовательно число подтверждений $n = \log_2{(1 + a)}$.

Отсюда получаем, что время для увеличения размера окна перегрузки не зависит от от средней пропускной способности, так как каждое подверждение занимает RTT времени.

Общее количество сегментов $S = W / 2 + (W / 2) * (1 + a) + ... + (W / 2) * (1 + a) ^ {n}$.


Средний размер окна это $W_{mean} = S / (n + 1) = W / 2 \cdot (1 - (1 + a) ^ n) / (1 - (1 + a)) = \frac{W \cdot a}{2 ((1 + a) ^ n - 1)}$.

Получаем, что частота это $L = 1 / (W_{mean} \cdot RTT) = \frac{2 (1 + a) ^ n - 1}{W \cdot a \cdot RTT}$. То есть получаем, что L обратно пропорционально W.
...
#### Решение
$RTT_{FE}$ времени потратится на то, чтобы установить соединение. Затем $RTT_{BE}$ потратится на запрос к серверу и ответ на него. Так как окно большое, то ответ придет сразу весь за 1 раз. По условию, обычно нужно три окна для отправки ответа при медленном старте, откуда и получаем $3 \cdot RTT_{FE}$. время обработки запроса очевидно ни под какую задержку спрятать нельзя, поэтому еще его добавляем к сумме. Проссумировав все, получаем $4 \cdot RTT_{FE} + RTT_{BE} + \text{ время обработки}$.